...
def creat_dict(list_of_students, list_of_scores):
    result = zip(list_of_students, list_of_scores)
    result = dict(result)
    return result

a = creat_dict(students, scores)
# print(a)
...
converted = list(map(lambda num: num * conversion_rate, usd_prices))
# print(converted)
...
Oldschools = list(filter(lambda num: num >= 18, ages))
...
def PriceGrade(obj):
    updated_products = {key: int(value * 1.1) for key, value in obj.items()}
    return updated_products

c = PriceGrade(products)
# print(c)
'''
Task:
...
updated_numbers = [num ** 2 for num in numbers]
updated_numbers = list(filter(lambda num: num >= 50, updated_numbers))
